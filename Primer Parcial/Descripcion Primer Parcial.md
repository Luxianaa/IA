...
Se utilizo un enfoque recursivo para generar las predicciones: cada valor estimado se usa como entrada para predecir el siguiente dia, simulando la evolucion de la temperatura maxima en los proximos 30 dias.
Es util para poder calcular en especial los climas , saber como estara el clima el proximo dia basandonos en datos y en este caso en predicines, lo negativo es que, al ser predicciones se puede ir acumulando el error si las predicciones no son muy acertadas y no en datos reales.
La grafica que se muestra en el cuadernillo muestra  como evoluciona el clima con el paso de los dias hasta los 30 dias, por lo que se aprecia, no hay una grafica inestable, con una cuantas variaciones en los primeros dias.
